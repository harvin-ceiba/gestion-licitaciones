pipeline {
  ...
  // Sección que define las herramientas "preinstaladas" en Jenkins
  tools {
  ...
  */
  ...
  // Aquí comienzan los "etapas" del Pipeline
  stages{
    stage('Checkout') {
      steps{
        echo "------------>Checkout<------------"
    ...
    stage('Compile & Unit Tests') {
      steps{
        ...
        sh 'chmod +x ./licitaciones/gradlew'
		sh './licitaciones/gradlew --b ./licitaciones/build.gradle test'
      }
    }
    ...
    stage('Build') {
      steps {
        ...
        // sh './licitaciones/gradlew --b ./licitaciones/build.gradle build -x test'
      }
    }  
  }
